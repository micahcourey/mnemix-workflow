...
Examples:
  ...
  ./scripts/publish-release.sh 0.1.0 --dry-run
...
- Verify the new version on PyPI with \`python3 -m pip index versions mnemix-workflow\`.
- Verify both \`pip install mnemix-workflow\` and \`pipx install mnemix-workflow\` in a clean environment.
EOF
